...
class LaplaceLoginSync:
    def __init__(self, url: str, uuid: str, password: str, auth_key: str):
        self.url: str = url
        self.uuid: str = uuid
    ...
    def get_encrypted_data(self) -> Optional[str]:
        ...
        if self.check_connection():
            cookie_cloud_request = requests.get(
                urljoin(self.url, "get/" + self.uuid + "?auth=" + self.auth_key)
            )
        ...
        if encrypted_data is not None:
            try:
                decrypted_data = decrypt(
                    encrypted_data, self.get_the_key().encode("utf-8")
                ).decode("utf-8")
    ...
    def update_cookie(self, cookie: Dict[str, Any]) -> bool:
        ...
        encrypted_data = encrypt(
            raw_data.encode("utf-8"), self.get_the_key().encode("utf-8")
        ).decode("utf-8")
        cookie_cloud_request = requests.post(
            urljoin(self.url, "/update"),
